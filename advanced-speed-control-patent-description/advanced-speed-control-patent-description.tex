...
\title{\textbf{ 
	...
	\end{Large}\\[24pt]
	\begin{large}
	\textbf{Patented (PA109238IT01) by Davide Bagnara and Felix Baumgartner, \\
		property of Leitner SpA.}
	\end{large}
}
}
...
The present invention relates to a ropeway transportation system comprising a carrying rope to which cabins (hereinafter referred to as gondolas) are attached. The carrying rope is driven by a pulley arrangement including two pulleys, one of which is operatively connected to a motor actuator, see Figures~\ref{model_derivation_1}~and~\ref{ropeway_iii}.

\begin{figure}[H]
	\centering
	\includegraphics[width = 500pt, keepaspectratio]{figures/introduction/model_derivation_1.eps}
	...
	\label{model_derivation_1}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width = 325pt, keepaspectratio]{figures/introduction/ropeway_iii.eps}
	\captionsetup{width=0.75\textwidth, font=small}
	\caption{Draw of the transportation line with description of the main elements of the system.}
	\label{ropeway_iii}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width = 425pt, keepaspectratio]{figures/patent/line_sketch_1.eps}
	\captionsetup{width=0.75\textwidth, font=small}
...
In some installations the ropeway line is subjected to oscillations. These oscillation are intrinsically present on all ropeway system due to its nature, they can be \textbf{dominant} or negligible. Typical dominant oscillations are shown in Figure~\ref{small_oscillation_constraints}. 
\begin{figure}[H]
	\centering
	\includegraphics[width = 500pt, keepaspectratio]{figures/patent/small_oscillation_constraints.eps}
	\captionsetup{width=0.75\textwidth, font=small}
...
\noindent\textbf{Remark} - the presence of friction-less supports (towers) will be not accounted for the design of the equivalent model. \\

\noindent\textbf{Remark} - the invention illustrated in this document consist of a control system design which is able to reduce the amplitude of these oscillations.

\section{Approach to the problem}
\textbf{Problem description:}
\begin{myitemize_1}
	\item[--] the line of a given ropeway starts oscillating, that means the rope moves vertically ($u$-direction) with dominant pulsations;
	\item[--] the ropeway is actuated through a motor; the motor applies a torque to the pulley, and the pulley drives the rope without slipping, hence the pulley exerts a force $f$ on the rope as result of the torque of the motor;
	\item[--] in order to find a control strategy which minimize the vertical oscillations of the rope \textbf{a correlation} between linear displacement oscillations (vertical oscillations of the line) and the corresponding torsional displacement oscillations (torsional components acting on the shaft of the motor/pulley) \textbf{shall be derived};
	\item[--] as first approach the linear displacement oscillations will be analyzed using "small oscillation of a continuous string"; a PDE equation which can describe the vertical oscillations of the rope can be assumed of the form
	\begin{equation}
	...
	\end{equation}
	\item[--] once the equivalent representation of the rope as \textbf{linear string displacement model} has been defined an equivalent interpretation as \textbf{torsional string displacement model} is investigated;
...
\noindent\textbf{Remark} - another aspect of the proposed invention concerns the safety of the ropeway. The presence of intrinsically resonance (as stationary oscillation of the line) as well as the presence of deterministic monochromatic excitation\footnote{The term \textit{deterministic monochromatic excitation} refers to the presence of specific harmonic.} disturbance could bring the overall line to unbounded oscillations.    
...
\noindent\textbf{Statement} - The ropeway transmission line is modeled as a oscillating string (in terms of vertical displacements) with fixed-end constraints. \\

\noindent\textbf{Statement} - Based on string theory, ropeway operational conditions, and experimental measurements, a set of critical frequencies has been identified. From experimental results it is possible to state that the identified critical frequencies correspond to the string oscillation modes (or stationary solution of the fixed string).\\

\noindent\textbf{Statement} - The control problem for attenuating ropeway line oscillations involves designing a controller capable of damping the vertical oscillations, particularly when they occur at specific critical frequencies. \\

\noindent\textbf{Statement} - The concept behind the control architecture for damping ropeway line oscillations is based on the \textbf{isomorphism} between oscillating string theory and a chain of harmonic oscillators, where the oscillators are modeled as rotational masses coupled with torsional springs. \\
\begin{figure}[H]
	\centering
	\includegraphics[width = 325pt, keepaspectratio]{figures/patent/conceptual_idea_figure_1.eps}
	\captionsetup{width=0.75\textwidth, font=small}
...
\end{figure}
The conceptual layout of the control system here developed is shown in Figure~\ref{conceptual_idea_figure_1}. In accordance with string theory, the oscillations of the ropeway line can be approximated as a finite set of harmonic modes corresponding to a fundamental frequency (or stationary solution of the fixed ends oscillating string). These harmonic frequencies are subsequently mapped onto a chain of harmonic oscillators, within which the control strategy is developed. Each stationary oscillations of the equivalent sting, which mimic the ropeway line, is composed by a spatial-domain terms and by a time-domain terms which are decoupled. The time-domain terms can easily mapped into a pure time-domain equivalent system based on harmonic oscillators. \\
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 375pt, keepaspectratio]{figures/patent/isomorphism-1.eps}
	\captionsetup{width=0.75\textwidth, font=small}
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 375pt  , keepaspectratio]{figures/patent/isomorphism-2.eps}
	\captionsetup{width=0.75\textwidth, font=small}
...
\noindent\textbf{Remark} - The overall control architecture adopts a model-based approach, whereby the system's performance is inherently dependent on the accuracy of the mathematical model relative to the actual ropeway system. \\

\begin{figure}[H]
	\centering
	\includegraphics[width = 500pt, keepaspectratio]{figures/patent/conceptual_idea_figure_2.eps}
	\captionsetup{width=0.75\textwidth, font=small}
...
%\begin{figure}[H]
%	\centering
%	\includegraphics[width = 525pt, keepaspectratio]{figures/patent/small_oscillations_figure_1.eps}
%	\captionsetup{width=0.75\textwidth, font=small}
...
\begin{myitemize_1}
	...
	\item[--] $\mu\ \Big[\SI{}{\kilogram\per\meter}\Big]$ : linear density of the rope;
	\item[--] $T\ \Big[\SI{}{\newton}\Big]$ : tension of the rope;
\end{myitemize_1}
...
In this subsection the effect of monochromatic input excitation to the line is investigated. Secondary effect of the process of attaching and detaching the gondolas to the rope is the generation of a monochromatic trigger, see also Figure~\ref{trigger_1}
\begin{figure}[H]
	\centering
	\includegraphics[width = 325pt, keepaspectratio]{figures/patent/trigger_1.eps}
	\captionsetup{width=0.75\textwidth, font=small}
	\caption{Description of the nature of the external excitation.}
	\label{trigger_1}
\end{figure}

\noindent\textbf{Remark} - the process of attaching and detaching the gondolas to the rope generates a source of disturbance which is assumed of the form:
\begin{equation}
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 450pt, keepaspectratio]{figures/patent/conceptual_idea_figure_5.eps}
	\captionsetup{width=0.75\textwidth, font=small}
	\caption{The dominant string oscillations are mapped onto a chain of harmonic oscillators to facilitate control design.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 225pt, keepaspectratio]{figures/patent/conceptual_idea_figure_6a.eps}
	\captionsetup{width=0.75\textwidth, font=small}
	\caption{The dominant string oscillations are mapped onto a chain of harmonic oscillators to facilitate control design.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 285pt, keepaspectratio]{figures/patent/conceptual_idea_figure_6b.eps}
	\captionsetup{width=0.75\textwidth, font=small}
	\caption{The dominant string oscillations are mapped onto a chain of harmonic oscillators to facilitate control design.}
...
Once the state space representation of the dominant resonances have been defined as chain of coupled rotation springs, the design of the control system can start. In this patent proposal one specific control strategy is adopted. \\ 
...
The mass-spring-mass model is a valid approximation of a flexible shaft, and for the purpose of this work will be used as approximation of the ropeway system where a main natural frequency is dominant among the others. The 
...
\begin{figure}[H]
	...
	\caption{Simplified equivalent model of a ropeway system, for the case scenario of one dominant harmonic.}
...
\begin{figure}[H]
	...
	\captionsetup{width=0.5\textwidth, font=small}		
	\caption{Simplified equivalent model of a ropeway system.}
